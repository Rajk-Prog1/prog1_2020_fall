...
class MyGameController(controller.Controller):
    ...
    def build_objects(self):
        ...
        self.custom_dict["talon"] = deck.Deck(enums.DeckType.short, talon_pos, talon_offset, None)
        ...
        for i in range(4):
            oszlop = osztalyok.Oszlop(oszlop_pos,oszlop_offset,enums,enums.GrabPolicy.can_single_grab)
    ...
    def start_game(self):
        ...
        for asztalszam in range(1, 8):
            for kartyaszam in range(0, asztalszam):
    ...
    def process_mouse_event(self, pos, down, double_click):
            ...
            if self.custom_dict["talon"].is_clicked(pos):
                ...
                if kartya_ is not None:
                    if len(self.custom_dict["atlapoz"].cards) == 0:
                        ...
                        kartya_.flip()
                    elif len(self.custom_dict["atlapoz"].cards) != 0:
                        prevkartya = self.custom_dict["atlapoz"].pop_card(True)
                        self.custom_dict["atlapoz"].add_card(kartya_)
                        kartya_.flip()
                        self.custom_dict["purgatory"].add_card(prevkartya)
                
                elif kartya_ is None:
                    prevkartya = self.custom_dict["atlapoz"].pop_card(True)
                    self.custom_dict["purgatory"].add_card(prevkartya)
                    if len(self.custom_dict["talon"].cards) == 0:
                        if len(self.custom_dict["purgatory"].cards) == 0:
                            return
                        else:
                            self.custom_dict["purgatory"].move_all_cards(self.custom_dict["talon"])
                            return
                ...
                for i in self.rendered_objects:
                    ...
                    if kartya_ is not None:
                        for j in kartya_:
                            self.custom_dict["kez"].add_card(j)
        ...
        if not down:
            ...
            for asztal in self.custom_dict["asztal"]:
                if asztal.pos[0] <= mpos[0] <= asztal.pos[0] + 130 and asztal.pos[1] <= mpos[1] <= asztal.pos[1] + 170:
                    print("jo nyomon vagy")
                    kartya_ = asztal.try_grab_card(pos)
                    if kartya_ is not None:
                        for i in kartya_:
                            self.custom_dict["kez"].add_card(i)                  
